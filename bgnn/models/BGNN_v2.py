...
class BGNN_v2(BaseModel):
    ...
    def update_node_features(self, node_features, X, encoded_X, all_mask):
        if self.task == 'regression':
    ...
    def update_gbdt_targets(self, node_features, node_features_before, train_mask):
        # try:
        #     if self.residual_before is None:
        #         raise            
        #     residual = (node_features - node_features_before).detach().cpu().numpy()[train_mask, -self.out_dim:]
        #     # apply dropout
        #     dropout_indices = (np.random.rand(*residual.shape) < 1-self.gnn_dropout).astype("int")
        #     residual = np.where(dropout_indices, residual, self.residual_before)
        #     # print("1")
    ...
    def fit(self, networkx_graph, X, y, train_mask, val_mask, test_mask, cat_features,
        ...
        for epoch in pbar:
            ...
            # gbdt part
            # print("gbdt_x_train: ", gbdt_X_train.shape)
            ...
            self.update_node_features(node_features, X, encoded_X, all_mask)
            ...
            self.residual_before = gbdt_y_train.copy()
